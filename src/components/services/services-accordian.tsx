  ...
  const ContentSection = () => (
    <div className="space-y-8">
      <div className="space-y-6">
        <h2 className="text-4xl md:text-5xl font-bold font-['Asgard'] text-primary">
          {title}
        </h2>
        <p className="text-lg 2xl:text-xl text-primary font-['Exo'] leading-relaxed max-w-2xl">
          {description}
        </p>
      </div>
      ...
      <div className="space-y-2">
        {accordionItems.map((item, index) => (
          <div key={index} className="border-none">
            <Link  href={`${item.href}`} className="group flex items-center justify-between py-4 px-0 border-b hover:border-[#00FC09] cursor-pointer">
         
                <div
                 
                  className="text-2xl 2xl:text-[30px] font-['Exo'] font-bold text-blue transition-colors hover:text-blue-600"
                >
                  {item.title}
                </div>
